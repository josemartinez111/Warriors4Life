...
<template>
  <div @click="handleCalendarClick" class="calendar-container">
    ...
    <div class="container mx-auto px-8">
      ...
      <div
        class="wrapper responsive-container">
        <!-- Header section with responsive font size -->
        <div class="header flex justify-between border-b px-2 py-3">
          <span class="text-sm lg:text-lg font-semibold">{{ currentMonth }} {{ currentYear
                                                         }}</span>
          <span class="uppercase">Click me</span>
          <div class="buttons flex">
            <button class="p-1 mr-2">
              <svg width="20" height="20" fill="currentColor"
                   viewBox="0 0 16 16" class="bi bi-arrow-left-circle">
                <!-- Left arrow SVG -->
              </svg>
            </button>
            <button class="p-1">
              <svg width="20" height="20" fill="currentColor"
                   viewBox="0 0 16 16" class="bi bi-arrow-right-circle">
                <!-- Right arrow SVG -->
              </svg>
            </button>
          </div>
        </div>
        <!-- Table settings adjust text size across different breakpoints: 'text-[10px] sm:text-xs md:text-sm' -->
        <table class="w-full text-[10px] sm:text-xs md:text-sm">
          <thead>
            <tr>
              <!-- Headers for days of the week with responsive design -->
              <th v-for="dayName in daysOfWeek" :key="dayName"
                  class="py-1 border-b border-r last:border-r-0 text-center">
                {{ dayName }}
              </th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr v-for="(week, index) in weeks" :key="index"
              ...
              <td v-for="day in week.days" :key="day.date.toISOString()"
                  :class="[
